...
Leia `AGENTS.md` por completo, verifique `sha256sum -c design-contract.sha256` e confronte o código com AQ-WEB-1.1.4. Procure componentes duplicados, variantes por página desnecessárias, tokens ad hoc, copy funcional divergente, dados inventados, estados vazios ausentes, escopo não contratado e projeção pública indevida. Não faça mutações git.
